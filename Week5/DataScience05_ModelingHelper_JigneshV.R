...
SlowandClean <-function(dataset,fractionOfTest=0.3)
{
  #generate random number
  randoms <- runif(nrow(dataset))
  # sort the random number
  sortedRandoms <- sort(randoms)
  #get the number from sortedRandoms which as the threshold based on the fractionoftest (e,g, 30th % value)
  testThreshold <- sortedRandoms[length(sortedRandoms)*fractionOfTest]
  # returns true for the 30% (fractionoftest) and false for rest
  testSelection <- randoms <= testThreshold
  # takes subset from the original set based on the fraction, this set is test set
  testSet <- dataset[testSelection, ] 
  # take rest of as training set
  trainSet <- dataset[!testSelection, ] 
  # add both the set in the list
  datasetSplit <- list(trainSet=trainSet, testSet=testSet)
  return(datasetSplit)
}
