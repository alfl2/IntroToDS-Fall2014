...
#takes dataframe, and constant fractionOftest for quick and dirty split
QuickAndDirty <-function(dataSet, fractionOfTest = 0.3)
{
  
  #generate random number
  randoms<-runif(nrow(dataSet))
  #create flag to partition case
  testSelection <- randoms <=fractionOfTest
  #Apply flag for test data
  testSet <-dataSet[testSelection,]
  #Apply flag for train data
...
SlowAndExact <-function(dataSet, fractionOfTest = 0.3)
{
  ...
  #threshold where quantile is test fraction
