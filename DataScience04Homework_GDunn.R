...
setwd("C:/Users/jdunn1/Documents/UWDataScience/Class4/")
source("KMeansTest.R")
...
{ 
  # Get ridiculous values for the initial cluster ids (-1)
  clusterIDOld <- -1;
...
simpleDetermineCentroids <- function(points=testPoints, clusterID=testClusterID)
{
  ...
  numberOfClusters <- max(clusterID)
  
  # Create a matrix where each row is a centroid of 2 dimensions
  centroids <- matrix(nrow=numberOfClusters, ncol=2)

  # For loop through each cluster id 
  for (ik in 1:numberOfClusters)
  {
...
{
  # Get the number of centroids (# of centroids = # of rows)
  numberOfCentroids <- nrow(centroids)
  ...
  for(pointNo in 1:numberOfPoints)
  {
    ...
    # if you get 2 centroids and one is shielding, the other one gets an NA so handle that
    if (is.na(oneDistanceSquared))
    {
      distanceSquared[pointNo, centroidNo] <- inf  # make it the least desirable distance    
    } else
    {
